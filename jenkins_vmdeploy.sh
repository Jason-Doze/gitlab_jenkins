...
rsync -av -e "ssh -o StrictHostKeyChecking=no -i ./id_ed25519" --delete --exclude={'id_ed25519*','cloud-init.yaml','README.md','gitlab_vmdeploy','vm_destroy','commands.txt'} "$(pwd)" "$USER@$(multipass info relativepath |  grep IPv4  | awk '{print $2}'):/home/$USER" 
 
echo -e "\n\033[1;32m==== Execute gitlab_install.sh on VM ====\033[0m\n"
ssh -o StrictHostKeyChecking=no -i ./id_ed25519 "$USER@$(multipass info relativepath | grep IPv4 | awk '{ print $2 }') 'cd relativepath_leveltwo && bash jenkins_install.sh'"
